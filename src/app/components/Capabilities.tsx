          ...
          {capabilityPillars.map((pillar, index) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-[1.8rem] border border-[#00448b]/10 bg-white p-6 shadow-[0_22px_50px_-42px_rgba(0,68,139,0.22)]"
              >
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#00448b] text-white">
                    <Icon className="h-5 w-5" />
                  </div>
                  <p className="section-eyebrow">{pillar.title}</p>
                </div>

                <h3 className="font-display mt-5 text-3xl leading-tight tracking-[-0.06em] text-slate-950">
                  {pillar.title}
                </h3>
                ...
                <div className="mt-6 space-y-3">
                  {pillar.capabilities.map((capability) => (
                    <div
                      key={capability}
                      className="rounded-[1rem] bg-white px-4 py-3 text-sm leading-6 text-slate-600 ring-1 ring-[#00448b]/8"
                    >
                      {capability}
                    </div>
                  ))}
                ...
                <p className="mt-6 border-t border-[#00448b]/10 pt-5 text-sm leading-7 text-slate-700">
                  {pillar.outcome}
                </p>
              </motion.div>
            );
